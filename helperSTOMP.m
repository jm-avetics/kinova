...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    sigma = eye(nDiscretize);
    % Use the stompSamples function to generate nPaths noisy trajectories
    [theta_paths, em] = stompSamples(nPaths, sigma, theta);

    %% TODO: Calculate Local trajectory cost for each sampled trajectory
    % Use the stompTrajCost function to compute the local trajectory cost for each sampled trajectory
    Stheta = zeros(nPaths, nDiscretize);
    Qtheta = zeros(nPaths, 1);
    for k = 1 : nPaths
        % Get the kth sampled trajectory
        theta_k = theta_paths{k};
        % Compute the local trajectory cost and the total cost for the kth trajectory
        [Stheta(k, :), Qtheta(k)] = stompTrajCost(robot_struct, theta_k, R, voxel_world);
    end

    %% TODO: Given the local traj cost, update local trajectory probability
    % Use stompUpdateProb to compute the exponential trajectory probability
    trajProb = stompUpdateProb(Stheta);

    %% TODO: Compute delta theta (aka gradient estimator, the improvement of the delta)
    % Use stompDTheta to compute the estimated gradient
    ...
    % Use stompTrajCost to compute the overall cost of the new trajectory
    [~, Qtheta] = stompTrajCost(robot_struct, theta, R, voxel_world);
