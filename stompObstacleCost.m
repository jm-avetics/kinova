...
try
    % Get the signed distance values for each sphere center from the voxel world
    s = voxel_world_sEDT(sub2ind(world_size,idx(:,1),idx(:,2),idx(:,3)));
    % Compute the cost array for each sphere center using Eq (13)
    cost_array = max(0, radius + safety_margin - s).^2 .* vel.^2;
    % Sum up the cost array to get the total cost
    cost = sum(cost_array);
catch  % for debugging
    idx = ceil((sphere_centers-env_corner_vec)./voxel_world.voxel_size);
end
